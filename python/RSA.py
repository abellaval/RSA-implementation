...
def main(): 
    ...
    while True :
        mode = input("Enter e/d/q/s/h for encryption/decryption/quit/show-keys/help : ")
        if mode.upper() == 'Q' :
            return None
        if mode.upper() == 'S' :
            show(N,e,d,p,q,phi)
        ...
        if mode.upper() == 'E' :
            print("{}\n>>> Plaintext : {}\n>>> Ciphertext : {}\n{}\n".format('-'*20,text, Encrypt(text,e,N),'-'*20))
        if mode.upper() == 'D' :
            print("{}\n>>> Ciphertext : {}\n>>> Plaintext : {}\n{}\n".format('-'*20,text, Decrypt(text,d,N),'-'*20))
...
if __name__ == "__main__" :
    ...
    if 1 :
        ...
        k = 32
        ((N,e),(N,d),p,q,phi,t) = KeyGen(k) # Generation des clefs et c'est tout
