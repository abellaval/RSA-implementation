...
class EchangeClef(Scene):
    ...
    def construct(self):
        ...
        key_green_exch = ImageMobject(f"{HOME}KeyGreen.png").scale(0.3).next_to(bob_img.get_top() + np.array([-1, 0, 0]), UP)
        key_red_exch = ImageMobject(f"{HOME}KeyRed.png").scale(0.3).next_to(alice_img.get_top() + np.array([1, 0, 0]), UP)
        ...
        self.play(FadeIn(key_green_exch), FadeIn(key_red_exch))

        # play échange
        runtime = 2
        self.play(key_red_exch.animate.next_to(bob_img.get_top() + np.array([-1, 0, 0]), UP),
                key_green_exch.animate.next_to(alice_img.get_top() + np.array([1, 0, 0]), UP), run_time=runtime)
        self.wait(5)
